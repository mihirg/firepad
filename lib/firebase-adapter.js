...
firepad.FirebaseAdapter = (function (global) {
  ...
  function FirebaseAdapter (ref, userId, userColor, backupRef) {
    this.ref_ = ref;
    this.ready_ = false;
    this.firebaseCallbacks_ = [];
    this.zombie_ = false;
    this.backupRef_ = backupRef;
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    function doTransaction(revisionId, revisionData) {
      ...
      if (self.backupRef_) {
        ...
        self.backupRef_.child('history').child(revisionId).transaction(function(current) {
          if (current === null) {
            return revisionData;
          }
        }, function(error, committed, snapshot) {
